...
clang++ -std=c++20 -O2 -DNDEBUG -target arm64-apple-macosx14.0 -isysroot "$SDK_DIR" -I"$READER_DIR/libcbh/include" -I"$READER_DIR/libcbh/src" "$READER_DIR/main.cpp" "$READER_DIR"/libcbh/src/*.cpp -lsqlite3 -o "$CHECK_DIR/LucentChessCBH"
CLANG_MODULE_CACHE_PATH="$PROJECT_DIR/.build-local/clang" swiftc -interface-compiler-version 'Apple Swift version 6.3 effective-5.10 (swiftlang-6.3.0.123.4 clang-2100.0.123.2)' -sdk "$SDK_DIR" -target arm64-apple-macosx14.0 -parse-as-library -O \
 ...
 "$PROJECT_DIR/Sources/LucentChess/Services/ChessBaseImportService.swift" "$PROJECT_DIR/Sources/LucentChess/Services/DatabaseCatalog.swift" \
